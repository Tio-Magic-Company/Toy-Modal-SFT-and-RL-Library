...
def _(mo):
    mo.md(r"""
# Tutorial 502: Convert to a PEFT LoRA adapter

Build a PEFT-style adapter directory from downloaded Toy Modal weights.

Adapter export is useful when you want to serve a base model plus LoRA instead of a merged model.

> Please note: Placeholder adapter files prove packaging shape, not numeric equivalence to real PEFT weights.
...
2. Copy safetensors/bin weights when present.
3. Write placeholder manifests offline.
4. Prepare for vLLM/SGLang style loading.

Run it with Marimo:

```bash
python -m pip install -e '.[backend,tutorials]'
marimo edit docs/tutorials/notebooks/502_lora_adapter.py
```
...
def _(mo):
    mo.md(r"""
<details>
<summary>Understanding LoRA adapter packaging</summary>

A LoRA adapter package stores the small trained adapter separately from the base model. This is often cheaper to store and flexible for serving systems that can load a base model once and swap adapters.

Toy Modal can build a PEFT-shaped adapter directory from checkpoint artifacts. The tutorial focuses on structure: adapter config, adapter weights, and clear manifests when the current run only has placeholder data.
...
This mirrors the LoRA adapter notebook using Toy Modal export helpers.
...
@app.cell
def _():
    checkpoint_dir = "artifacts/tutorial-checkpoint"
    adapter_output_dir = "artifacts/tutorial-lora-adapter"
    adapter_config = {"r": 4, "lora_alpha": 8, "target_modules": ["q_proj", "v_proj"]}
    return adapter_config, adapter_output_dir, checkpoint_dir,
...
@app.cell
async def _(adapter_config, app_name, base_model, cost_ack, environment_name, mo, run_remote, tinker):
    mo.stop(not cost_ack.value or not run_remote.value, "Enable remote execution before saving adapter source weights.")
    service_client = tinker.ServiceClient(project_id="tutorial-502", transport="modal-direct", app_name=app_name.value, environment_name=environment_name.value or None)
    training = await service_client.create_lora_training_client_async(base_model=base_model.value, rank=adapter_config["r"])
    sampler = await training.save_weights_and_get_sampling_client_async("adapter-source")
    print("Saved sampler weights:", sampler.model_path)
    return sampler,
...
@app.cell
def _(adapter_config, adapter_output_dir, checkpoint_dir, weights):
    adapter_dir = weights.build_lora_adapter(checkpoint_dir, adapter_output_dir, adapter_config=adapter_config)
...
- Use 501 to merge into HF model folders.
- Use 503 to publish when validated.
- Load the adapter in your serving stack before release.
    """)
    return
